	...
	public partial class IfcTable : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcTableClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc2x3.UtilityResource.IfcTable"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcTable.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
