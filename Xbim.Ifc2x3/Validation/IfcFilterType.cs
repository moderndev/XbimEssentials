	...
	public partial class IfcFilterType : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcFilterTypeClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc2x3.HVACDomain.IfcFilterType"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcFilterType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
