	...
	public partial class IfcEdgeLoop : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcEdgeLoopClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc2x3.TopologyResource.IfcEdgeLoop"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcEdgeLoop.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
