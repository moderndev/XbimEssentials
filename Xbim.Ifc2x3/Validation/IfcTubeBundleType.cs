	...
	public partial class IfcTubeBundleType : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcTubeBundleTypeClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc2x3.HVACDomain.IfcTubeBundleType"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcTubeBundleType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
