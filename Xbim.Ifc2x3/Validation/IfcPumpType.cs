	...
	public partial class IfcPumpType : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcPumpTypeClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc2x3.HVACDomain.IfcPumpType"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcPumpType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
