	...
	public partial class IfcAddress : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcAddressClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc2x3.ActorResource.IfcAddress"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcAddress.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
