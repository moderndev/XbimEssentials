	...
	public partial class IfcConstraintRelationship : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcConstraintRelationshipClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc2x3.ConstraintResource.IfcConstraintRelationship"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcConstraintRelationship.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
