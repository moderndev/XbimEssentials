	...
	public partial class IfcExternalReference : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcExternalReferenceClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc2x3.ExternalReferenceResource.IfcExternalReference"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcExternalReference.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
