	...
	public partial class IfcPhysicalComplexQuantity : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcPhysicalComplexQuantityClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc2x3.QuantityResource.IfcPhysicalComplexQuantity"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcPhysicalComplexQuantity.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
