	...
	public partial class IfcGasTerminalType : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcGasTerminalTypeClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc2x3.HVACDomain.IfcGasTerminalType"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcGasTerminalType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
