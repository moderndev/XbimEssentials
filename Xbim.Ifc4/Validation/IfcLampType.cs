	...
	public partial class IfcLampType : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcLampTypeClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ElectricalDomain.IfcLampType"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcLampType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
