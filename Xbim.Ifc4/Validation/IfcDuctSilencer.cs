	...
	public partial class IfcDuctSilencer : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcDuctSilencerClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.HvacDomain.IfcDuctSilencer"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcDuctSilencer.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
