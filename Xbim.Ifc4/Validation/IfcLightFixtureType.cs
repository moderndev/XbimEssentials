	...
	public partial class IfcLightFixtureType : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcLightFixtureTypeClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ElectricalDomain.IfcLightFixtureType"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcLightFixtureType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
