	...
	public partial class IfcColumnStandardCase : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcColumnStandardCaseClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.SharedBldgElements.IfcColumnStandardCase"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcColumnStandardCase.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
