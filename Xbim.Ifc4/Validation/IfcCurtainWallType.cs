	...
	public partial class IfcCurtainWallType : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcCurtainWallTypeClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.SharedBldgElements.IfcCurtainWallType"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCurtainWallType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
