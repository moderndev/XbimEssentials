	...
	public partial class IfcMedicalDevice : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcMedicalDeviceClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.HvacDomain.IfcMedicalDevice"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcMedicalDevice.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
