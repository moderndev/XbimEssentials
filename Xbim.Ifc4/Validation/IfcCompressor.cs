	...
	public partial class IfcCompressor : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcCompressorClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.HvacDomain.IfcCompressor"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCompressor.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
