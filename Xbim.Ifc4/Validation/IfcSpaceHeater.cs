	...
	public partial class IfcSpaceHeater : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcSpaceHeaterClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.HvacDomain.IfcSpaceHeater"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcSpaceHeater.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
