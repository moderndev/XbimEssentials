	...
	public partial class IfcElectricDistributionBoardType : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcElectricDistributionBoardTypeClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ElectricalDomain.IfcElectricDistributionBoardType"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcElectricDistributionBoardType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
