	...
	public partial class IfcUnitaryControlElement : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcUnitaryControlElementClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.BuildingControlsDomain.IfcUnitaryControlElement"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcUnitaryControlElement.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
