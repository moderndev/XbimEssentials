	...
	public partial class IfcComplexPropertyTemplate : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcComplexPropertyTemplateClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.Kernel.IfcComplexPropertyTemplate"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcComplexPropertyTemplate.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
