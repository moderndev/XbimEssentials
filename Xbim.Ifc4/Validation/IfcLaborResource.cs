	...
	public partial class IfcLaborResource : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcLaborResourceClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ConstructionMgmtDomain.IfcLaborResource"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcLaborResource.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
