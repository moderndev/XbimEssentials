	...
	public partial class IfcMechanicalFastenerType : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcMechanicalFastenerTypeClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.SharedComponentElements.IfcMechanicalFastenerType"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcMechanicalFastenerType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
