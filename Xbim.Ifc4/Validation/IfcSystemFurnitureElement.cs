	...
	public partial class IfcSystemFurnitureElement : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcSystemFurnitureElementClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.SharedFacilitiesElements.IfcSystemFurnitureElement"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcSystemFurnitureElement.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
