	...
	public partial class IfcBoiler : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcBoilerClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.HvacDomain.IfcBoiler"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcBoiler.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
