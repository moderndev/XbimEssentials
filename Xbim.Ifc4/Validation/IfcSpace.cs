	...
	public partial class IfcSpace : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcSpaceClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ProductExtension.IfcSpace"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcSpace.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
