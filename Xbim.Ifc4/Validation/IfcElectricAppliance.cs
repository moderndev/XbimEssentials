	...
	public partial class IfcElectricAppliance : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcElectricApplianceClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ElectricalDomain.IfcElectricAppliance"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcElectricAppliance.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
