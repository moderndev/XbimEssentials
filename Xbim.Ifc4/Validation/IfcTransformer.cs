	...
	public partial class IfcTransformer : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcTransformerClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ElectricalDomain.IfcTransformer"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcTransformer.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
