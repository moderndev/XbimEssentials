	...
	public partial class IfcTubeBundle : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcTubeBundleClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.HvacDomain.IfcTubeBundle"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcTubeBundle.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
