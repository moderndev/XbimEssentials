	...
	public partial class IfcFeatureElementSubtraction : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcFeatureElementSubtractionClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ProductExtension.IfcFeatureElementSubtraction"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcFeatureElementSubtraction.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
