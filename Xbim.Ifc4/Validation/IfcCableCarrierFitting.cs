	...
	public partial class IfcCableCarrierFitting : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcCableCarrierFittingClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ElectricalDomain.IfcCableCarrierFitting"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcCableCarrierFitting.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
