	...
	public partial class IfcEvaporativeCooler : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcEvaporativeCoolerClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.HvacDomain.IfcEvaporativeCooler"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcEvaporativeCooler.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
