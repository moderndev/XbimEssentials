	...
	public partial class IfcVibrationIsolator : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcVibrationIsolatorClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.HvacDomain.IfcVibrationIsolator"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcVibrationIsolator.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
