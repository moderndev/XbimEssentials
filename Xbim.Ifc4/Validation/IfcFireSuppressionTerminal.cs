	...
	public partial class IfcFireSuppressionTerminal : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcFireSuppressionTerminalClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.PlumbingFireProtectionDomain.IfcFireSuppressionTerminal"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcFireSuppressionTerminal.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
