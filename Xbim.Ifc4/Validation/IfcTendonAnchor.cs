	...
	public partial class IfcTendonAnchor : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcTendonAnchorClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.StructuralElementsDomain.IfcTendonAnchor"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcTendonAnchor.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
