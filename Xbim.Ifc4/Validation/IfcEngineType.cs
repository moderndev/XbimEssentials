	...
	public partial class IfcEngineType : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcEngineTypeClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.HvacDomain.IfcEngineType"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcEngineType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
