	...
	public partial class IfcFlowMeter : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcFlowMeterClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.HvacDomain.IfcFlowMeter"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcFlowMeter.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
