	...
	public partial class IfcRelConnectsPorts : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcRelConnectsPortsClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ProductExtension.IfcRelConnectsPorts"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcRelConnectsPorts.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
