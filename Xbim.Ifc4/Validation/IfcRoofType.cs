	...
	public partial class IfcRoofType : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcRoofTypeClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.SharedBldgElements.IfcRoofType"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcRoofType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
