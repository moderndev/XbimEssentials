	...
	public partial class IfcOutletType : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcOutletTypeClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ElectricalDomain.IfcOutletType"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcOutletType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
