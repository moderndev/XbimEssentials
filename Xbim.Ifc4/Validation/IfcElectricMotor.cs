	...
	public partial class IfcElectricMotor : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcElectricMotorClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ElectricalDomain.IfcElectricMotor"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcElectricMotor.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
