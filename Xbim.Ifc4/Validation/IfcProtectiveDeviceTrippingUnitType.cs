	...
	public partial class IfcProtectiveDeviceTrippingUnitType : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcProtectiveDeviceTrippingUnitTypeClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ElectricalDomain.IfcProtectiveDeviceTrippingUnitType"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcProtectiveDeviceTrippingUnitType.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
