	...
	public partial class IfcOutlet : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcOutletClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.ElectricalDomain.IfcOutlet"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcOutlet.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
