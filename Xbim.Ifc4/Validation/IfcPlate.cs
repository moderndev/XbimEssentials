	...
	public partial class IfcPlate : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcPlateClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.SharedBldgElements.IfcPlate"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcPlate.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
