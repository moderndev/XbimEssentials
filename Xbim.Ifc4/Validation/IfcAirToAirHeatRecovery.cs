	...
	public partial class IfcAirToAirHeatRecovery : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcAirToAirHeatRecoveryClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.HvacDomain.IfcAirToAirHeatRecovery"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcAirToAirHeatRecovery.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
