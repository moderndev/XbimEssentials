	...
	public partial class IfcAirTerminal : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcAirTerminalClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.HvacDomain.IfcAirTerminal"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcAirTerminal.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
