	...
	public partial class IfcAirTerminalBox : IExpressValidatable
	{
		...
		public bool ValidateClause(IfcAirTerminalBoxClause clause) {
			...
			try
			{
				...
				var Log = LogManager.GetLogger(Type.GetType("Xbim.Ifc4.HvacDomain.IfcAirTerminalBox"));
				Log.Error(string.Format("Exception thrown evaluating where-clause 'IfcAirTerminalBox.{0}' for #{1}.", clause,EntityLabel), ex);
			}
			return retVal;
		}
